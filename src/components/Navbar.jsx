...
const Navbar = ({
  ...
  return (
    <div className="bg-gradient-to-b from-[#2b2b7b] to-[#1b1b51] md:bg-none md:bg-[#2b2b7b] flex items-center justify-evenly md:justify-between md:px-15 lg:py-1 lg:justify-center lg:gap-8">
      <div className="left">
        <i
          className={`bi ${isMenuOpen ? "bi-x" : "bi-list"} text-white text-[45px] lg:hidden`}
          onClick={() => setIsMenuOpen(!isMenuOpen)}
        ></i>
        <img src={Logo} alt="Logo" className="hidden lg:block w-60 p-3 cursor-pointer" />
      </div>
      ...
      <div ref={searchRef} className="relative">
        <div
          className={`p-1 overflow-hidden ${
            isExpanded ? "w-[210px] md:w-[390px] lg:w-[450px] xl:w-[600px]" : "w-[40px] md:w-[320px] xl:w-[500px]"
        ...
        <div className="relative hidden md:block" ref={dropdownRef}>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-md py-2 z-50 flex flex-col items-center">
              {user ? (
                <>
                  <button onClick={logout} className="w-full px-4 py-2 text-black hover:bg-gray-300">Cerrar sesión</button>
                  <button className="w-full px-4 py-2 text-black hover:bg-gray-300">Cambiar contraseña</button>
                </>
              ) : (
                <>
                  <button onClick={openLoginModal} className="w-full px-4 py-2 text-black hover:bg-gray-300">Ingresar</button>
                  <button onClick={openRegisterModal} className="w-full px-4 py-2 text-black hover:bg-gray-300">Crear cuenta</button>
                </>
              )}
            </div>
          )}
      ...
      <MobileMenu isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} onRegisterClick={openRegisterModal} onLoginClick={openLoginModal} />

      {isMenuOpen && <div className="fixed inset-0 bg-[#00000057] bg-opacity-50 z-40 lg:hidden" onClick={() => setIsMenuOpen(false)}></div>}
    </div>
  );
};
